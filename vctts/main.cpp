...
static SpeakOutcome SpeakWithFallback(const std::wstring& text, bool preferKeyless)
{
    SpeakOutcome outcome;
    if (!g_state) {
        outcome.error = L"TTS state is not initialized.";
        return outcome;
    }

    if (g_state->ttsEngine == TtsEngine::Custom)
    {
        outcome.audio = custom_tts::SpeakCustomCommand(text, g_state->customTtsCommand);
        if (outcome.audio.empty()) {
            outcome.error = L"Custom command produced an empty audio buffer.";
        }
        return outcome;
    }

    if (g_state->ttsEngine == TtsEngine::Phonomenal)
    {
        const auto bankDirectory = std::filesystem::path(custom_tts::Utf8ToWide(g_state->phonomenalBankDirectory));
        const auto bankPath = tts_phonomenal::bank_path_for_index(bankDirectory, g_state->phonomenalVoiceIndex);
        auto result = tts_phonomenal::speak_wav(text, bankPath);
        { std::lock_guard lock(g_state->phonomenalStatusMutex); g_state->phonomenalStatus = custom_tts::WideToUtf8(result.status); }
        outcome.audio = std::move(result.audio);
    ...
    if (preferKeyless)
    {
        outcome.audio = tts_keyless::speak_to_audio_memory(text);
        if (!outcome.audio.empty()) return outcome;
        outcome.audio = tts_winrt::speak_wav(text);
        if (outcome.audio.empty()) {
            outcome.error = L"Both keyless backup and system TTS returned empty audio.";
        }
        return outcome;
    }
    ...
    outcome.audio = tts_keyless::speak_to_audio_memory(text);
    if (outcome.audio.empty()) {
        outcome.error = L"Both system TTS and keyless backup returned empty audio.";
    }
    return outcome;
}
...
static void OnCommittedText(const std::wstring& text)
{
    ...
    std::thread([copy, preferKeyless]() {
        auto result = SpeakWithFallback(SanitizeForSapi(copy), preferKeyless);
        if (result.audio.empty()) {
            const std::wstring& message = result.error.empty() ? L"TTS produced an invalid/empty audio buffer." : result.error;
            MessageBoxW(nullptr, message.c_str(), L"TTS Error", MB_ICONERROR);
            return;
        }
        audio_playback::play_wav_to_selected_async(result.audio, *g_state);
    }).detach();
}
    ...
    while (!state.exitRequested.load())
    {
        ...
        else if (action == UiAction::TestTts)
        {
            std::thread([]{
                ...
                const bool preferKeyless = g_state->useKeylessBackup.load();
                const std::wstring testPhrase =
                    g_state->ttsEngine == TtsEngine::Phonomenal ? L"Need medic right here." : L"Test text to speech.";
                auto result = SpeakWithFallback(testPhrase, preferKeyless);
                ...
                if (audio.empty()) {
                    const std::wstring& message = result.error.empty()
                        ? L"TTS produced an invalid/empty audio buffer. Check DebugView output."
                        : result.error;
                    MessageBoxW(nullptr, message.c_str(), L"TTS Error", MB_ICONERROR);
                    return;
                }
